...
function LocationAutocomplete({ label, value, onChange, placeholder }: { label: string; value: string; onChange: (value: string) => void; placeholder: string }) {
  ...
  return <label className="home-v2-location-field"><span>{label}</span><div><MapPin /><input value={value} onChange={(event) => { onChange(event.target.value); setIsOpen(true); setActiveIndex(0); }} onFocus={() => setIsOpen(true)} onBlur={() => { blurTimeout.current = window.setTimeout(() => setIsOpen(false), 150); }} onKeyDown={(event) => { if (event.key === "ArrowDown") { event.preventDefault(); setIsOpen(true); setActiveIndex((current) => Math.min(current + 1, matches.length - 1)); } if (event.key === "ArrowUp") { event.preventDefault(); setActiveIndex((current) => Math.max(current - 1, 0)); } if (event.key === "Escape") setIsOpen(false); if (event.key === "Enter" && isOpen && matches[activeIndex]) { event.preventDefault(); selectLocation(matches[activeIndex].name); } }} placeholder={placeholder} role="combobox" aria-expanded={isOpen} aria-controls={listId} aria-autocomplete="list" aria-activedescendant={isOpen && matches[activeIndex] ? `${listId}-${activeIndex}` : undefined} /></div>{isOpen && <div className="location-suggestions" id={listId} role="listbox">{matches.length ? matches.map((option, index) => <button key={`${option.name}-${option.detail}`} id={`${listId}-${index}`} type="button" role="option" aria-selected={activeIndex === index} className={activeIndex === index ? "is-active" : ""} onMouseDown={(event) => event.preventDefault()} onClick={() => selectLocation(option.name)}><MapPin /><span><b>{option.name}</b><small>{option.detail}</small></span><em>{option.code}</em></button>) : <p>No city or station matches. Continue typing.</p>}</div>}</label>;
}
